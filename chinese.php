...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'http://chinese.cnu.edu.cn/xzhd/index.htm');
...
foreach($matches[3] as $key => $val){
	$id = trim($val);
	$pre = $pdo->prepare("select * from scholar where type='chinese' and scholarid=:scholarid");
	...
	if($res === false){
        $ch1 = curl_init();
        curl_setopt($ch1, CURLOPT_URL, 'http://chinese.cnu.edu.cn/xzhd/'.$id.'.htm');
        curl_setopt($ch1, CURLOPT_HEADER, 0);
        curl_setopt($ch1, CURLOPT_RETURNTRANSFER, 1);
        $content1 = curl_exec($ch1);
        curl_close($ch1);
        if(empty($content1)) continue;
        preg_match('/<div class="article([\s\S]*?)<\/div>/i', $content1, $match1);
        ...
        var_dump($id,$matches[2][$key],$date);
		$pre = $pdo->prepare("insert into scholar (`scholarid`,`type`,`tittle`,`date`,`indate`) values(:scholarid,'chinese',:tittle,:date,'".date('Y-m-d')."')");
		$pre->bindParam(':scholarid',$id);
